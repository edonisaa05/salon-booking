/* ═══════════════════════════════════════════════════════
   ...
   Terminet ruhen në PostgreSQL nëpërmjet API C#
═══════════════════════════════════════════════════════ */
...
function doRegister() {
  ...
  // Check against hardcoded admins + registered users
  const allEmails = [
...
async function renderUserBookings() {
  ...
  container.innerHTML = '<p style="color:var(--ink-muted);font-size:.9rem">Duke ngarkuar...</p>';

  let bookings = [];
  try {
    const resp = await fetch(`${API_BASE}/bookings`);
    const all  = await resp.json();
    // Filtro vetëm terminet e këtij useri (sipas emrit)
    bookings = all.filter(b => b.customerName === currentUser.name);
  } catch {
    container.innerHTML = '<p style="color:red;font-size:.9rem">Gabim: Nuk u lidh me serverin.</p>';
    return;
  }
  ...
  container.innerHTML = bookings.map(b => {
    ...
    const dateStr = dt.toLocaleDateString('sq-AL', { day: '2-digit', month: '2-digit', year: 'numeric' });
    const timeStr = dt.toLocaleTimeString('sq-AL', { hour: '2-digit', minute: '2-digit' });
    // API nuk ka status - i trajtojmë si Confirmed
    const statusClass = 'status-confirmed';
    const statusLabel = '✓ Konfirmuar';
    const canCancel = true;

    return `
      <div class="booking-card" id="bcard-${b.id}">
        <div class="booking-card-header">
          <span class="booking-service">${escHtml(b.serviceName)}</span>
        ...
        <div class="booking-card-footer">
          ${canCancel
            ...
            : `<span style="font-size:.8rem;color:var(--ink-muted)">Statusi: Konfirmuar</span>`
          }
        </div>
      </div>`;
  }).join('');
...
async function cancelUserBooking(id) {
  ...
  try {
    const resp = await fetch(`${API_BASE}/bookings/${id}`, { method: 'DELETE' });
    if (!resp.ok) {
      showToast('Gabim gjatë anulimit të terminit.');
      return;
    }
    ...
    showToast('Nuk mund të lidhet me serverin.');
  }
}
...
async function submitBooking() {
  ...
  // Kombinojmë datën dhe orën në format datetime
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      showBookingError(msg || 'Gabim gjatë rezervimit. Provoni përsëri.');
      return;
    }
    ...
    showBookingError('Nuk mund të lidhet me serverin. A është i ndezur API?');
    console.error(err);
  }
}
...
async function renderAdminBookings(filter = '') {
  ...
  tbody.innerHTML = '<tr><td colspan="7" style="color:var(--ink-muted);padding:1rem">Duke ngarkuar...</td></tr>';
  ...
  try {
    ...
    tbody.innerHTML = '<tr><td colspan="7" style="color:red;padding:1rem">Gabim: Nuk u lidh me serverin.</td></tr>';
    return;
  }
  ...
  tbody.innerHTML = bookings.map(b => {
    ...
    return `
      <tr id="arow-${b.id}">
        <td>#${b.id}</td>
        <td>${escHtml(b.customerName)}</td>
        <td>${escHtml(b.serviceName)}</td>
        <td>${dateStr}</td>
        <td>${timeStr}</td>
        <td><span class="status-badge status-confirmed">Konfirmuar</span></td>
        <td>
  ...
  // Ruaj bookings në memorie për modalin
...
async function adminDeleteBooking(id) {
  ...
  try {
    const resp = await fetch(`${API_BASE}/bookings/${id}`, { method: 'DELETE' });
    if (!resp.ok) { showToast('Gabim gjatë fshirjes.'); return; }
...
function openDetailModal(id) {
  const b = (window._adminBookings || []).find(b => b.id === id);
  if (!b) return;
  ...
  document.getElementById('modal-body').innerHTML = `
    <div class="modal-row"><span class="label">Klienti</span><span class="value">${escHtml(b.customerName)}</span></div>
    <div class="modal-row"><span class="label">Shërbimi</span><span class="value">${escHtml(b.serviceName)}</span></div>
    <div class="modal-row"><span class="label">Data</span><span class="value">${dateStr}</span></div>
    <div class="modal-row"><span class="label">Ora</span><span class="value">${timeStr}</span></div>
    <div class="modal-row"><span class="label">Statusi</span><span class="value">Konfirmuar</span></div>
  `;
...
function openEditModal(id) {
  ...
  const serviceOptions = services.map(s =>
    `<option value="${escHtml(s.name)}" ${b.serviceName === s.name ? 'selected' : ''}>${escHtml(s.name)}</option>`
  ).join('');

  const timeOptions = ['09:00','09:30','10:00','10:30','11:00','11:30','12:00','12:30',
    '13:00','13:30','14:00','14:30','15:00','15:30','16:00','16:30','17:00','17:30']
    .map(t => `<option value="${t}" ${timeVal === t ? 'selected' : ''}>${t}</option>`).join('');
...
async function saveEditBooking(id) {
  ...
  try {
    ...
    if (!resp.ok) { showToast('Gabim gjatë përditësimit.'); return; }
